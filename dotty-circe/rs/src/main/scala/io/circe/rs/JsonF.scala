...
import io.circe.cats.{ Applicative, Eval, Traverse }
...
object JsonF {
  ...
  private[this] val vectorInstance: Traverse[Vector] = the[Traverse[Vector]]
  private[this] val fieldInstance: Traverse[Fields] = vectorInstance.compose[Field] given Traverse[Field]
  ...
  implicit val jsonFTraverseInstance: Traverse[JsonF] = new Traverse[JsonF] {
    override def traverse[G[_], A, B](fa: JsonF[A])(f: A => G[B])(implicit G: Applicative[G]): G[JsonF[B]] = fa match {
      ...
      case JArrayF(vecA)    => G.map(vectorInstance.traverse(vecA)(f))(vecB => JArrayF(vecB))
    ...
    override def foldRight[A, B](fa: JsonF[A], lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
      fa match {
        ...
        case JStringF(_)   => lb
        case JNumberF(_)   => lb
        case JArrayF(vecA) => vectorInstance.foldRight(vecA, lb)(f)
        case JObjectF(fieldsA) =>
