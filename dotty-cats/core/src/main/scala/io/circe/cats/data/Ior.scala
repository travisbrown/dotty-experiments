...
import io.circe.cats.{Alternative, Applicative, Bifunctor, Bitraverse, Eval, Monad, MonadError, Parallel, Show, Traverse}
import io.circe.cats.arrow.FunctionK
import io.circe.cats.data.Validated.{Invalid, Valid}
import io.circe.cats.kernel.{Eq, Order, Semigroup}
...
sealed abstract class Ior[+A, +B] extends Product with Serializable {
  ...
  final def toIorNes[AA >: A] given Order[AA]: IorNes[AA, B] = leftMap(NonEmptySet.one(_))
  ...
  final def to[F[_], BB >: B] given (F: Alternative[F]): F[BB] =
  ...
  final def valueOr[BB >: B](f: A => BB) given (BB: Semigroup[BB]): BB =
  ...
  final def flatMap[AA >: A, D](f: B => AA Ior D) given (AA: Semigroup[AA]): AA Ior D = this match {
    case l @ Ior.Left(_) => l
  ...
  final def traverse[F[_], AA >: A, D](g: B => F[D]) given (F: Applicative[F]): F[AA Ior D] = this match {
  ...
  final def combine[AA >: A, BB >: B](that: AA Ior BB) given (AA: Semigroup[AA], BB: Semigroup[BB]): AA Ior BB =
    this match {
      case Ior.Left(a1) =>
        that match {
          case Ior.Left(a2)     => Ior.Left(AA.combine(a1, a2))
  ...
  final def ===[AA >: A, BB >: B](that: AA Ior BB) given (AA: Eq[AA], BB: Eq[BB]): Boolean = fold(
  ...
  final def show[AA >: A, BB >: B] given (AA: Show[AA], BB: Show[BB]): String = fold(
...
sealed abstract private[data] class IorInstances extends IorInstances0 {
  ...
  implicit val catsBitraverseForIor: Bitraverse[Ior] = new Bitraverse[Ior] {
    ...
    def bitraverse[G[_], A, B, C, D](fab: Ior[A, B])(f: A => G[C],
                                                     g: B => G[D]) given (G: Applicative[G]): G[Ior[C, D]] =
      fab match {
  ...
  implicit def catsDataMonadErrorForIor[A: Semigroup]: MonadError[[a] =>> Ior[A, a], A] =
    new MonadError[[a] =>> Ior[A, a], A] {
  ...
  // scalastyle:off cyclomatic.complexity
  implicit def catsDataParallelForIor[E] given (E: Semigroup[E]): Parallel[[a] =>> Ior[E, a], [a] =>> Ior[E, a]] =
    new Parallel[[a] =>> Ior[E, a], [a] =>> Ior[E, a]] {

      private[this] val identityK: FunctionK[[a] =>> Ior[E, a], [a] =>> Ior[E, a]] = FunctionK.id

      def parallel: FunctionK[[a] =>> Ior[E, a], [a] =>> Ior[E, a]] = identityK
      def sequential: FunctionK[[a] =>> Ior[E, a], [a] =>> Ior[E, a]] = identityK

      val applicative: Applicative[[a] =>> Ior[E, a]] = new Applicative[[a] =>> Ior[E, a]] {
      ...
      lazy val monad: Monad[[a] =>> Ior[E, a]] = Monad[[a] =>> Ior[E, a]]
    }
  // scalastyle:on cyclomatic.complexity
...
sealed abstract private[data] class IorInstances0 {
  ...
  implicit def catsDataTraverseFunctorForIor[A]: Traverse[[a] =>> Ior[A, a]] = new Traverse[[a] =>> Ior[A, a]] {
