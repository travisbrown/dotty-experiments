...
import io.circe.cats.{Alternative, Always, Applicative, CoflatMap, Eval, Monad, Now, Show, Traverse, TraverseFilter}
import io.circe.cats.kernel.{Eq, Monoid}
import scala.annotation.tailrec
import scala.language.implicitConversions

private[cats] object StreamInstance extends Traverse[Stream] with Alternative[Stream] with Monad[Stream] with CoflatMap[Stream] with TraverseFilter[Stream] {
  ...
  override def foldMap[A, B](fa: Stream[A])(f: A => B) given (B: Monoid[B]): B =
    B.combineAll(fa.iterator.map(f))

  def traverse[G[_], A, B](fa: Stream[A])(f: A => G[B]) given (G: Applicative[G]): G[Stream[B]] =
  ...
  override def foldM[G[_], A, B](fa: Stream[A], z: B)(f: (B, A) => G[B]) given (G: Monad[G]): G[B] = {
  ...
  override def fold[A](fa: Stream[A]) given (A: Monoid[A]): A = A.combineAll(fa)
  ...
  override def algebra[A]: Monoid[Stream[A]] = the[Monoid[Stream[A]]]
  ...
  val traverse: Traverse[Stream] = this
  ...
  def traverseFilter[G[_], A, B](fa: Stream[A])(f: (A) => G[Option[B]]) given (G: Applicative[G]): G[Stream[B]] =
    fa.foldRight(Eval.now(G.pure(Stream.empty[B])))(
        (x, xse) => G.map2Eval(f(x), xse)((i, o) => i.fold(o)(_ +: o))
      )
      .value

  override def filterA[G[_], A](fa: Stream[A])(f: (A) => G[Boolean]) given (G: Applicative[G]): G[Stream[A]] =
