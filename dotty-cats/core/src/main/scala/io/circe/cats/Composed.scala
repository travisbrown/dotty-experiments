...
private[cats] trait ComposedDistributive[F[_], G[_]] extends Distributive[[α] =>> F[G[α]]] with ComposedFunctor[F, G] {
  outer =>
...
private[cats] trait ComposedInvariant[F[_], G[_]] extends Invariant[[α] =>> F[G[α]]] { outer =>
  def F: Invariant[F]
  def G: Invariant[G]
...
private[cats] trait ComposedFunctor[F[_], G[_]] extends Functor[[α] =>> F[G[α]]] with ComposedInvariant[F, G] { outer =>
  def F: Functor[F]
  def G: Functor[G]
...
private[cats] trait ComposedApply[F[_], G[_]] extends Apply[[α] =>> F[G[α]]] with ComposedFunctor[F, G] { outer =>
  def F: Apply[F]
  def G: Apply[G]

  override def ap[A, B](fgf: F[G[A => B]])(fga: F[G[A]]): F[G[B]] =
    F.ap[G[A], G[B]](F.map(fgf)(gf => ga => G.ap(gf)(ga)))(fga)

  override def product[A, B](fga: F[G[A]], fgb: F[G[B]]): F[G[(A, B)]] =
    F.map2(fga, fgb)(G.product)
}

private[cats] trait ComposedApplicative[F[_], G[_]] extends Applicative[[α] =>> F[G[α]]] with ComposedApply[F, G] {
  outer =>
  def F: Applicative[F]
  def G: Applicative[G]
...
private[cats] trait ComposedSemigroupK[F[_], G[_]] extends SemigroupK[[α] =>> F[G[α]]] { outer =>
  def F: SemigroupK[F]

  override def combineK[A](x: F[G[A]], y: F[G[A]]): F[G[A]] = F.combineK(x, y)
}

private[cats] trait ComposedMonoidK[F[_], G[_]] extends MonoidK[[α] =>> F[G[α]]] with ComposedSemigroupK[F, G] { outer =>
  def F: MonoidK[F]
...
private[cats] trait ComposedAlternative[F[_], G[_]]
    extends Alternative[[α] =>> F[G[α]]]
    with ComposedApplicative[F, G]
    with ComposedMonoidK[F, G] { outer =>
  def F: Alternative[F]
}

private[cats] trait ComposedFoldable[F[_], G[_]] extends Foldable[[α] =>> F[G[α]]] { outer =>
  def F: Foldable[F]
  def G: Foldable[G]
...
private[cats] trait ComposedNonEmptyTraverse[F[_], G[_]]
    extends NonEmptyTraverse[[α] =>> F[G[α]]]
...
private[cats] trait ComposedReducible[F[_], G[_]] extends Reducible[[α] =>> F[G[α]]] with ComposedFoldable[F, G] { outer =>
  def F: Reducible[F]
  def G: Reducible[G]
...
private[cats] trait ComposedContravariant[F[_], G[_]] extends Functor[[α] =>> F[G[α]]] { outer =>
...
private[cats] trait ComposedContravariantCovariant[F[_], G[_]] extends Contravariant[[α] =>> F[G[α]]] { outer =>
  def F: Contravariant[F]
  def G: Functor[G]
...
private[cats] trait ComposedApplicativeContravariantMonoidal[F[_], G[_]]
    extends ContravariantMonoidal[[α] =>> F[G[α]]] { outer =>
  def F: Applicative[F]
...
private[cats] trait ComposedSemigroupal[F[_], G[_]]
    extends ContravariantSemigroupal[[α] =>> F[G[α]]]
...
private[cats] trait ComposedInvariantApplySemigroupal[F[_], G[_]]
    extends InvariantSemigroupal[[α] =>> F[G[α]]]
...
private[cats] trait ComposedCovariantContravariant[F[_], G[_]] extends Contravariant[[α] =>> F[G[α]]] { outer =>
  def F: Functor[F]
  def G: Contravariant[G]
...
private[cats] trait ComposedInvariantCovariant[F[_], G[_]] extends Invariant[[α] =>> F[G[α]]] { outer =>
  def F: Invariant[F]
  def G: Functor[G]
...
private[cats] trait ComposedInvariantContravariant[F[_], G[_]] extends Invariant[[α] =>> F[G[α]]] { outer =>
  def F: Invariant[F]
  def G: Contravariant[G]
