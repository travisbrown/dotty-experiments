...
trait ApplicativeError[F[_], E] extends Applicative[F] {
   ...
   * and raise.
   */
  def catchNonFatal[A](a: => A) given (E: Throwable <:< E): F[A] =
    try pure(a)
    catch {
      case NonFatal(e) => raiseError(E(e))
    }
   ...
   * and raise
   */
  def catchNonFatalEval[A](a: Eval[A]) given (E: Throwable <:< E): F[A] =
    try pure(a.value)
    catch {
      case NonFatal(e) => raiseError(E(e))
    }
   ...
   */
  def fromTry[A](t: Try[A]) given (E: Throwable <:< E): F[A] =
    t match {
      case Success(a) => pure(a)
      case Failure(e) => raiseError(E(e))
    }
