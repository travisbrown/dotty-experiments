...
import io.circe.cats.{Applicative, Apply, Bifoldable, CommutativeApplicative, CommutativeApply, Contravariant, ContravariantMonoidal, Eval, Functor, Show, Traverse, TraverseFilter}
import io.circe.cats.kernel.{CommutativeMonoid, CommutativeSemigroup, Eq, LowerBounded, Monoid, Order, PartialOrder, Semigroup, UpperBounded}
...
case class Const[A, B](getConst: A) {
  ...
  def combine(that: Const[A, B]) given (A: Semigroup[A]): Const[A, B] =
    Const(A.combine(getConst, that.getConst))

  def traverse[F[_], C](f: B => F[C]) given (F: Applicative[F]): F[Const[A, C]] =
    F.pure(retag[C])

  def ===(that: Const[A, B]) given (A: Eq[A]): Boolean =
    A.eqv(getConst, that.getConst)

  def partialCompare(that: Const[A, B]) given (A: PartialOrder[A]): Double =
    A.partialCompare(getConst, that.getConst)

  def compare(that: Const[A, B]) given (A: Order[A]): Int =
    A.compare(getConst, that.getConst)

  def show given (A: Show[A]): String =
    s"Const(${A.show(getConst)})"
}

object Const extends ConstInstances {
  def empty[A, B] given (A: Monoid[A]): Const[A, B] =
    Const(A.empty)
...
sealed abstract private[data] class ConstInstances extends ConstInstances0 {
  given [A, B] as UpperBounded[Const[A, B]] given (A: UpperBounded[A]) {
  ...
  implicit def catsDataTraverseForConst[C]: Traverse[[x] =>> Const[C, x]] = new Traverse[[x] =>> Const[C, x]] {
  ...
  implicit def catsDataTraverseFilterForConst[C]: TraverseFilter[[x] =>> Const[C, x]] = new TraverseFilter[[x] =>> Const[C, x]] {
    ...
    val traverse: Traverse[[x] =>> Const[C, x]] = Const.catsDataTraverseForConst[C]
  }
...
sealed abstract private[data] class ConstInstances0 extends ConstInstances1 {
  ...
  implicit def catsDataContravariantMonoidalForConst[D: Monoid]: ContravariantMonoidal[[x] =>> Const[D, x]] =
    new ContravariantMonoidal[[x] =>> Const[D, x]] {
  ...
  ): CommutativeApplicative[[x] =>> Const[C, x]] =
    new ConstApplicative[C] with CommutativeApplicative[[x] =>> Const[C, x]] { val C0: CommutativeMonoid[C] = C }
}

sealed abstract private[data] class ConstInstances1 extends ConstInstances2 {

  implicit def catsDataCommutativeApplyForConst[C](implicit C: CommutativeSemigroup[C]): CommutativeApply[[x] =>> Const[C, x]] =
    new ConstApply[C] with CommutativeApply[[x] =>> Const[C, x]] { val C0: CommutativeSemigroup[C] = C }
}
...
sealed abstract private[data] class ConstInstances2 extends ConstInstances3 {
  ...
  implicit def catsDataApplicativeForConst[C](implicit C: Monoid[C]): Applicative[[x] =>> Const[C, x]] =
...
sealed abstract private[data] class ConstInstances3 extends ConstInstances4 {
  ...
  implicit def catsDataApplyForConst[C](implicit C: Semigroup[C]): Apply[[x] =>> Const[C, x]] =
    new ConstApply[C] { val C0: Semigroup[C] = C }
}

sealed abstract private[data] class ConstInstances4 {

  implicit def catsDataFunctorForConst[C]: Functor[[x] =>> Const[C, x]] =
    new ConstFunctor[C] {}

  implicit def catsDataContravariantForConst[C]: Contravariant[[x] =>> Const[C, x]] =
    new ConstContravariant[C] {}
}

sealed private[data] trait ConstFunctor[C] extends Functor[[x] =>> Const[C, x]] {
  def map[A, B](fa: Const[C, A])(f: A => B): Const[C, B] =
    fa.retag[B]
}

sealed private[data] trait ConstContravariant[C] extends Contravariant[[x] =>> Const[C, x]] {
  override def contramap[A, B](fa: Const[C, A])(f: B => A): Const[C, B] =
    fa.retag[B]
}

sealed private[data] trait ConstApply[C] extends ConstFunctor[C] with Apply[[x] =>> Const[C, x]] {
...
sealed private[data] trait ConstApplicative[C] extends ConstApply[C] with Applicative[[x] =>> Const[C, x]] {
