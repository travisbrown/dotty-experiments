...
private[cats] object ListInstance extends Traverse[List] with Alternative[List] with Monad[List] with CoflatMap[List] with TraverseFilter[List] {
  ...
  override def foldMap[A, B](fa: List[A])(f: A => B) given (B: Monoid[B]): B =
    B.combineAll(fa.iterator.map(f))

  def traverse[G[_], A, B](fa: List[A])(f: A => G[B]) given (G: Applicative[G]): G[List[B]] =
  ...
  override def partitionEither[A, B, C](
    fa: List[A]
  )(f: (A) => Either[B, C]) given (A: Alternative[List]): (List[B], List[C]) =
  ...
  override def foldM[G[_], A, B](fa: List[A], z: B)(f: (B, A) => G[B]) given (G: Monad[G]): G[B] = {
  ...
  override def fold[A](fa: List[A]) given (A: Monoid[A]): A = A.combineAll(fa)
  ...
  override def algebra[A]: Monoid[List[A]] = the[Monoid[List[A]]]
  ...
  val traverse: Traverse[List] = this
  ...
  def traverseFilter[G[_], A, B](fa: List[A])(f: (A) => G[Option[B]]) given (G: Applicative[G]): G[List[B]] =
    fa.foldRight(Eval.now(G.pure(List.empty[B])))(
        (x, xse) => G.map2Eval(f(x), xse)((i, o) => i.fold(o)(_ :: o))
      )
      .value

  override def filterA[G[_], A](fa: List[A])(f: (A) => G[Boolean]) given (G: Applicative[G]): G[List[A]] =
