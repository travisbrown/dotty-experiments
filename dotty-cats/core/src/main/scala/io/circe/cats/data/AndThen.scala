...
import io.circe.cats.{Contravariant, ContravariantMonoidal, Monad}
    ...
    while (continue) {
      self match {
        case Concat(left, inner) =>
          ...
          right = inner.andThenF(right).asInstanceOf[AndThen[Any, Any]]
...
abstract private[data] class AndThenInstances0 extends AndThenInstances1 {
   ...
   */
  given [T] as Monad[[x] =>> AndThen[T, x]] {
      // Piggybacking on the instance for Function1
      private[this] val fn1 = the[Monad[[x] =>> T => x]]
   ...
   */
  given [R] as ContravariantMonoidal[[x] =>> AndThen[x, R]] given Monoid[R] {
      // Piggybacking on the instance for Function1
      private[this] val fn1 = the[ContravariantMonoidal[[x] =>> x => R]]
