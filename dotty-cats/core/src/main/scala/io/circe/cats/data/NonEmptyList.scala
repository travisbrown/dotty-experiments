...
import io.circe.cats.{Always, Applicative, Apply, Bimonad, CommutativeApply, Eval, FlatMap, Foldable, NonEmptyParallel, NonEmptyReducible, NonEmptyTraverse, Reducible, SemigroupK, Show, Traverse, ~>}
import io.circe.cats.data.NonEmptyList.ZipNonEmptyList
import io.circe.cats.kernel.{Eq, Order, Monoid, PartialOrder, Semigroup}
...
final case class NonEmptyList[+A](head: A, tail: List[A]) {
  ...
  def foldRight[B](lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
    the[Foldable[List]].foldRight(toList, lb)(f)
  ...
  def ===[AA >: A](o: NonEmptyList[AA])(implicit AA: Eq[AA]): Boolean = {
    AA.eqv(this.head: AA, o.head) && Eq[List[AA]].eqv(this.tail: List[AA], o.tail)
  }
    ...
    with NonEmptyTraverse[NonEmptyList] {
      ...
      override def nonEmptyPartition[A, B, C](
        ...
        val lastIor = f(reversed.head) match {
          case Left(a) => Ior.left(NonEmptyList.one(a))
          case Right(b) => Ior.right(NonEmptyList.one(b))
        }
  ...
  implicit def catsDataNonEmptyParallelForNonEmptyList[A]: NonEmptyParallel[NonEmptyList, ZipNonEmptyList] =
    new NonEmptyParallel[NonEmptyList, ZipNonEmptyList] {
      ...
      def sequential: ZipNonEmptyList ~> NonEmptyList =
        new (ZipNonEmptyList ~> NonEmptyList) { def apply[X](x: ZipNonEmptyList[X]) = x.value }

      def parallel: NonEmptyList ~> ZipNonEmptyList =
        new (NonEmptyList ~> ZipNonEmptyList) { def apply[X](x: NonEmptyList[X]) = new ZipNonEmptyList(x) }
    }
}
