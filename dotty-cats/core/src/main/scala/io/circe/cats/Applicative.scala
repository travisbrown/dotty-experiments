...
trait Applicative[F[_]] extends Apply[F] with InvariantMonoidal[F] { self =>
  ...
  def replicateA[A](n: Int, fa: F[A]): F[List[A]] =
    the[Traverse[List]].sequence(List.fill(n)(fa)) given this
...
object Applicative {
  def apply[F[_]] given (F: Applicative[F]): Applicative[F] = F
  ...
  private[cats] trait Ops {
    given [F[_], A] {
      def (fa: F[A]) replicateA(n: Int) given (F: Applicative[F]): F[List[A]] = F.replicateA(n, fa)
      def (fa: F[A]) unlessA(cond: Boolean) given (F: Applicative[F]): F[Unit] = F.unlessA(cond)(fa)
      def (fa: F[A]) whenA(cond: Boolean) given (F: Applicative[F]): F[Unit] = F.whenA(cond)(fa)
    }

    given [A] {
      def (a: A) pure[F[_]] given (F: Applicative[F]): F[A] = F.pure(a)
    }
  }
}

private[cats] class ApplicativeMonoid[F[_], A] given (F: Applicative[F], A: Monoid[A])
    extends ApplySemigroup[F, A]
    with Monoid[F[A]] {
  def empty: F[A] = F.pure(A.empty)
}

private[cats] class ArrowApplicative[F[_, _], A] given (F: Arrow[F]) extends Applicative[[x] =>> F[A, x]] {
