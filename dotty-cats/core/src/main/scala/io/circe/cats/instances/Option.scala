...
import io.circe.cats.{Alternative, Applicative, CoflatMap, CommutativeMonad, Eval, MonadError, Now, Show, Traverse, TraverseFilter}
import io.circe.cats.kernel.{Monoid, Order}
import scala.annotation.tailrec
import scala.language.implicitConversions

private[cats] object OptionInstance extends Traverse[Option] with MonadError[Option, Unit] with Alternative[Option] with CommutativeMonad[Option]
  with CoflatMap[Option] with TraverseFilter[Option] {
  ...
  override def minimumOption[A](fa: Option[A]) given (A: Order[A]): Option[A] = fa

  override def maximumOption[A](fa: Option[A]) given (A: Order[A]): Option[A] = fa
  ...
  override def foldMap[A, B](fa: Option[A])(f: A => B) given (B: Monoid[B]): B =
    fa.fold(B.empty)(f)
  ...
  val traverse: Traverse[Option] = this
  ...
  def traverseFilter[G[_], A, B](fa: Option[A])(f: (A) => G[Option[B]]) given (G: Applicative[G]): G[Option[B]] =
    fa match {
      case None    => G.pure(Option.empty[B])
      case Some(a) => f(a)
    }

  override def filterA[G[_], A](fa: Option[A])(f: (A) => G[Boolean]) given (G: Applicative[G]): G[Option[A]] =
    fa match {
