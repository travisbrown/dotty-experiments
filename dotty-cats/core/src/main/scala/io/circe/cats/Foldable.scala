...
import io.circe.cats.kernel.{CommutativeMonoid, Monoid, Order}
import io.circe.cats.Foldable.sentinel
 ...
 */
trait Foldable[F[_]] extends UnorderedFoldable[F] { self =>
   ...
   */
  def minimumOption[A](fa: F[A]) given (A: Order[A]): Option[A] =
   ...
   */
  def maximumOption[A](fa: F[A]) given (A: Order[A]): Option[A] =
  ...
  def get[A](fa: F[A])(idx: Long): Option[A] = {
    if (idx < 0L) None
      ...
      foldM[[x] =>> Either[A, x], A, Long](fa, 0L) { (i, a) =>
   ...
   */
  def fold[A](fa: F[A]) given (A: Monoid[A]): A =
   ...
   */
  def foldMap[A, B](fa: F[A])(f: A => B) given (B: Monoid[B]): B =
   ...
   */
  def foldM[G[_], A, B](fa: F[A], z: B)(f: (B, A) => G[B]) given (G: Monad[G]): G[B] = {
    val src = Foldable.Source.fromFoldable(fa) given this
    G.tailRecM((z, src)) {
      case (b, src) =>
        src.uncons match {
   ...
   * Alias for [[foldM]].
   */
  final def foldLeftM[G[_], A, B](fa: F[A], z: B)(f: (B, A) => G[B]) given Monad[G]: G[B] =
    foldM(fa, z)(f)
   ...
   * }}}
   */
  def foldMapM[G[_], A, B](fa: F[A])(f: A => G[B]) given (G: Monad[G], B: Monoid[B]): G[B] =
   ...
   * needed.
   */
  def traverse_[G[_], A, B](fa: F[A])(f: A => G[B]) given (G: Applicative[G]): G[Unit] =
   ...
   * }}}
   */
  def sequence_[G[_], A](fga: F[G[A]]) given Applicative[G]: G[Unit] =
   ...
   * }}}
   */
  def foldK[G[_], A](fga: F[G[A]]) given (G: MonoidK[G]): G[A] =
    fold(fga) given G.algebra
   ...
   * }}}
   */
  def existsM[G[_], A](fa: F[A])(p: A => G[Boolean]) given (G: Monad[G]): G[Boolean] =
    G.tailRecM(Foldable.Source.fromFoldable(fa) given this) { src =>
      src.uncons match {
   ...
   * }}}
   */
  def forallM[G[_], A](fa: F[A])(p: A => G[Boolean]) given (G: Monad[G]): G[Boolean] =
    G.tailRecM(Foldable.Source.fromFoldable(fa) given this) { src =>
      src.uncons match {
   ...
   * }}}
   */
  def partitionEither[A, B, C](fa: F[A])(f: A => Either[B, C]) given (A: Alternative[F]): (F[B], F[C]) = {
    val B: Monoid[F[B]] = A.algebra[B]
    val C: Monoid[F[C]] = A.algebra[C]
    ...
    foldMap(fa)(
      ...
      Monoid.instance(
        (B.empty, C.empty),
        {
          case ((fb1, fc1), (fb2, fc2)) => (B.combine(fb1, fb2), C.combine(fc1, fc2))
        }
      )
    )
  }
   ...
   * }}}
   */
  def intercalate[A](fa: F[A], a: A) given (A: Monoid[A]): A =
  ...
  def compose[G[_]] given Foldable[G]: Foldable[[x] =>> F[G[x]]] =
    new ComposedFoldable[F, G] {
      val F = self
      val G = the[Foldable[G]]
    }

  override def unorderedFold[A](fa: F[A]) given CommutativeMonoid[A]: A = fold(fa)

  override def unorderedFoldMap[A, B](fa: F[A])(f: (A) => B) given CommutativeMonoid[B]: B =
    foldMap(fa)(f)
}
...
object Foldable {
  ...
  private[cats] trait Ops {
    ...
    given [F[_], G[_], A] given (F: Foldable[F]) {
      def (fga: F[G[A]]) sequence_ given Applicative[G]: G[Unit] = F.sequence_(fga)
    }
  }
  ...
  private[cats] object Source {
    ...
    def fromFoldable[F[_], A](fa: F[A]) given (F: Foldable[F]): Source[A] =
