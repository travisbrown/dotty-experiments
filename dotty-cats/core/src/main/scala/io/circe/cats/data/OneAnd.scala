...
import io.circe.cats.{Alternative, Always, Applicative, Apply, Comonad, Eval, Foldable, Functor, Monad, NonEmptyReducible, NonEmptyTraverse, Parallel, Reducible, SemigroupK, Show, Traverse, ~>}
...
sealed abstract private[data] class OneAndInstances extends OneAndLowPriority0 {
  ...
  ): Parallel[[a] =>> OneAnd[M, a], [a] =>> OneAnd[F, a]] =
    new Parallel[[a] =>> OneAnd[M, a], [a] =>> OneAnd[F, a]] {
      def monad: Monad[[a] =>> OneAnd[M, a]] = catsDataMonadForOneAnd(P.monad, Alternative[M])

      def applicative: Applicative[[a] =>> OneAnd[F, a]] = catsDataApplicativeForOneAnd(Alternative[F])

      def sequential: FunctionK[[a] =>> OneAnd[F, a], [a] =>> OneAnd[M, a]] =
        new FunctionK[[a] =>> OneAnd[F, a], [a] =>> OneAnd[M, a]] {
          def apply[X](x: OneAnd[F, X]) = OneAnd(x.head, P.sequential(x.tail))
        }

      def parallel: FunctionK[[a] =>> OneAnd[M, a], [a] =>> OneAnd[F, a]] =
        new FunctionK[[a] =>> OneAnd[M, a], [a] =>> OneAnd[F, a]] {
          def apply[X](x: OneAnd[M, X]) = OneAnd(x.head, P.parallel(x.tail))
        }
  ...
  implicit def catsDataSemigroupKForOneAnd[F[_]: Alternative]: SemigroupK[[a] =>> OneAnd[F, a]] =
    new SemigroupK[[a] =>> OneAnd[F, a]] {
  ...
  implicit def catsDataMonadForOneAnd[F[_]](implicit monad: Monad[F],
                                            alternative: Alternative[F]): Monad[[a] =>> OneAnd[F, a]] =
    new Monad[[a] =>> OneAnd[F, a]] {
...
sealed abstract private[data] class OneAndLowPriority4 {
  implicit val catsDataComonadForNonEmptyStream: Comonad[[a] =>> OneAnd[Stream, a]] =
    new Comonad[[a] =>> OneAnd[Stream, a]] {
...
sealed abstract private[data] class OneAndLowPriority3 extends OneAndLowPriority4 {
  ...
  implicit def catsDataFunctorForOneAnd[F[_]](implicit F: Functor[F]): Functor[[a] =>> OneAnd[F, a]] =
    new Functor[[a] =>> OneAnd[F, a]] {
...
sealed abstract private[data] class OneAndLowPriority2 extends OneAndLowPriority3 {
  ...
  implicit def catsDataApplicativeForOneAnd[F[_]](implicit F: Alternative[F]): Applicative[[a] =>> OneAnd[F, a]] =
    new Applicative[[a] =>> OneAnd[F, a]] {
      override def map[A, B](fa: OneAnd[F, A])(f: A => B): OneAnd[F, B] =
        fa.map(f)
...
sealed abstract private[data] class OneAndLowPriority1 extends OneAndLowPriority2 {
  ...
  implicit def catsDataTraverseForOneAnd[F[_]](implicit F: Traverse[F]): Traverse[[a] =>> OneAnd[F, a]] =
    new Traverse[[a] =>> OneAnd[F, a]] {
...
sealed abstract private[data] class OneAndLowPriority0_5 extends OneAndLowPriority1 {
  implicit def catsDataReducibleForOneAnd[F[_]](implicit F: Foldable[F]): Reducible[[a] =>> OneAnd[F, a]] =
    new NonEmptyReducible[[a] =>> OneAnd[F, a], F] {
...
sealed abstract private[data] class OneAndLowPriority0 extends OneAndLowPriority0_5 {
  implicit def catsDataNonEmptyTraverseForOneAnd[F[_]](implicit F: Traverse[F],
                                                       F2: Alternative[F]): NonEmptyTraverse[[a] =>> OneAnd[F, a]] =
    new NonEmptyReducible[[a] =>> OneAnd[F, a], F] with NonEmptyTraverse[[a] =>> OneAnd[F, a]] {
      def nonEmptyTraverse[G[_], A, B](fa: OneAnd[F, A])(f: (A) => G[B])(implicit G: Apply[G]): G[OneAnd[F, B]] = {
        ...
        fa.map(a => Apply[G].map(f(a))(OneAnd(_, F2.empty[B])))(F)
          .reduceLeft((acc, a) => G.map2(acc, a)(_.combine(_)))// => (acc, a).mapN((x: OneAnd[F, B], y: OneAnd[F, B]) => x.combine(y)))
      }
