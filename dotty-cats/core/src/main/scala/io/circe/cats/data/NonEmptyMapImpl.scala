...
import io.circe.cats.{Always, Apply, Eval, Foldable, Functor, Later, NonEmptyTraverse, Now, SemigroupK, Show}
import io.circe.cats.kernel.{Band, Eq, Monoid, Order, Semigroup}
...
sealed class NonEmptyMapOps[K, A](val value: NonEmptyMap[K, A]) {
  ...
  def map[B](f: A => B): NonEmptyMap[K, B] =
    NonEmptyMapImpl.create(Functor[[a] =>> SortedMap[K, a]].map(toSortedMap)(f))
  ...
  def foldRight[B](lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
    Foldable[[a] =>> SortedMap[K, a]].foldRight(toSortedMap, lb)(f)
  ...
  def reduceRightTo[B](f: A => B)(g: (A, Eval[B]) => Eval[B]): Eval[B] =
    Always((head, tail)).flatMap {
      case ((_, a), ga) =>
        Foldable[[a] =>> SortedMap[K, a]].reduceRightToOption(ga)(f)(g).flatMap {
...
sealed abstract private[data] class NonEmptyMapInstances {
  ...
  implicit def catsDataInstancesForNonEmptyMap[K: Order]
    : SemigroupK[[a] =>> NonEmptyMap[K, a]] with NonEmptyTraverse[[a] =>> NonEmptyMap[K, a]] =
    new SemigroupK[[a] =>> NonEmptyMap[K, a]] with NonEmptyTraverse[[a] =>> NonEmptyMap[K, a]] {
