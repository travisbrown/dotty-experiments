...
trait Apply[F[_]] extends Functor[F] with InvariantSemigroupal[F] with ApplyArityFunctions[F] { self =>
   ...
   * }}}
   */
  def compose[G[_]] given Apply[G]: Apply[[x] =>> F[G[x]]] =
...
object Apply {
   ...
   */
  def semigroup[F[_], A] given Apply[F], Semigroup[A]: Semigroup[F[A]] =
    new ApplySemigroup[F, A]

  private[cats] trait Ops {    
    given [F[_], A, B] given (F: Apply[F]) {
