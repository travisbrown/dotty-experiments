...
import io.circe.cats.{Applicative, ApplicativeError, Bitraverse, CommutativeApplicative, Eval, NotNull, Now, SemigroupK, Show, Traverse}
...
sealed abstract private[data] class ValidatedInstances extends ValidatedInstances1 {
  ...
  implicit def catsDataSemigroupKForValidated[A](implicit A: Semigroup[A]): SemigroupK[[a] =>> Validated[A, a]] =
    new SemigroupK[[a] =>> Validated[A, a]] {
  ...
  implicit def catsDataApplicativeErrorForValidated[E](implicit E: Semigroup[E]): ApplicativeError[[a] =>> Validated[E, a], E] =
    new ValidatedApplicative[E] with ApplicativeError[[a] =>> Validated[E, a], E] {
...
sealed abstract private[data] class ValidatedInstances1 extends ValidatedInstances2 {
  ...
  implicit def catsDataCommutativeApplicativeForValidated[E: CommutativeSemigroup]
    : CommutativeApplicative[[a] =>> Validated[E, a]] =
    new ValidatedApplicative[E] with CommutativeApplicative[[a] =>> Validated[E, a]]
...
sealed abstract private[data] class ValidatedInstances2 {
  ...
  implicit def catsDataTraverseFunctorForValidated[E]: Traverse[[a] =>> Validated[E, a]] =
    new Traverse[[a] =>> Validated[E, a]] {
...
private[data] class ValidatedApplicative[E: Semigroup] extends CommutativeApplicative[[a] =>> Validated[E, a]] {
  override def map[A, B](fa: Validated[E, A])(f: A => B): Validated[E, B] =
    fa.map(f)
