...
import io.circe.cats.{Alternative, Always, Applicative, CoflatMap, Eval, Monad, Show, Traverse, TraverseFilter}
import io.circe.cats.kernel.{Eq, Monoid, Order}
import scala.annotation.tailrec
import scala.collection.+:
import scala.collection.immutable.VectorBuilder

private[cats] object VectorInstance extends Traverse[Vector] with Monad[Vector] with Alternative[Vector] with CoflatMap[Vector] with TraverseFilter[Vector] {
  ...
  override def foldMap[A, B](fa: Vector[A])(f: A => B) given (B: Monoid[B]): B =
  ...
  override def traverse[G[_], A, B](fa: Vector[A])(f: A => G[B]) given (G: Applicative[G]): G[Vector[B]] =
  ...
  override def foldM[G[_], A, B](fa: Vector[A], z: B)(f: (B, A) => G[B]) given (G: Monad[G]): G[B] = {
  ...
  override def fold[A](fa: Vector[A]) given (A: Monoid[A]): A = A.combineAll(fa)
  ...
  override def algebra[A]: Monoid[Vector[A]] = the[Monoid[Vector[A]]]
  ...
  val traverse: Traverse[Vector] = this
  ...
  def traverseFilter[G[_], A, B](fa: Vector[A])(f: (A) => G[Option[B]]) given (G: Applicative[G]): G[Vector[B]] =
    fa.foldRight(Eval.now(G.pure(Vector.empty[B])))(
        (x, xse) => G.map2Eval(f(x), xse)((i, o) => i.fold(o)(_ +: o))
      )
      .value

  override def filterA[G[_], A](fa: Vector[A])(f: (A) => G[Boolean]) given (G: Applicative[G]): G[Vector[A]] =
