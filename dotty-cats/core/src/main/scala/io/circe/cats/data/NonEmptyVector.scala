...
import io.circe.cats.{Always, Applicative, Apply, Bimonad, CommutativeApply, Eval, FlatMap, Foldable, NonEmptyParallel, NonEmptyReducible, NonEmptyTraverse, Reducible, Show, SemigroupK, Traverse, ~>}
  ...
  implicit val catsDataInstancesForNonEmptyVector: SemigroupK[NonEmptyVector]
    ...
    with NonEmptyTraverse[NonEmptyVector] {
      ...
      override def nonEmptyPartition[A, B, C](
        ...
        reduceLeftTo(fa)(a => f(a) match {
          case Left(a) => Ior.left(NonEmptyVector.one(a))
          case Right(b) => Ior.right(NonEmptyVector.one(b))
        })(
          (ior, a) =>
            (f(a), ior) match {
        ...
        ).bimap(toNonEmptyList, toNonEmptyList)
  ...
  implicit def catsDataParallelForNonEmptyVector[A]: NonEmptyParallel[NonEmptyVector, ZipNonEmptyVector] =
    new NonEmptyParallel[NonEmptyVector, ZipNonEmptyVector] {
      ...
      def sequential: ZipNonEmptyVector ~> NonEmptyVector =
        new (ZipNonEmptyVector ~> NonEmptyVector) { def apply[X](x: ZipNonEmptyVector[X]) = x.value }

      def parallel: NonEmptyVector ~> ZipNonEmptyVector =
        new (NonEmptyVector ~> ZipNonEmptyVector) { def apply[X](x: NonEmptyVector[X]) = new ZipNonEmptyVector(x) }
    }
