...
import io.circe.cats.kernel.laws.discipline._
...
object KernelSuite extends DisciplineSuite {
  ...
  checkAll(
    "PartialOrder.reverse(PartialOrder[Set[Int]])",
    (PartialOrderTests given PartialOrder.reverse(PartialOrder[Set[Int]])).partialOrder
  )
  checkAll(
    "PartialOrder.reverse(PartialOrder.reverse(PartialOrder[Set[Int]]))",
    (PartialOrderTests given PartialOrder.reverse(PartialOrder.reverse(PartialOrder[Set[Int]]))).partialOrder
  )
  ...
  checkAll(
    "Semilattice.asMeetPartialOrder[Set[Int]]",
    (PartialOrderTests given Semilattice.asMeetPartialOrder[Set[Int]]).partialOrder
  )
  checkAll(
    "Semilattice.asJoinPartialOrder[Set[Int]]",
    (PartialOrderTests given Semilattice.asJoinPartialOrder[Set[Int]]).partialOrder
  )
  ...
  checkAll("fromOrdering[Int]", (OrderTests given Order.fromOrdering[Int]).order)
  checkAll("Order.reverse(Order[Int])", (OrderTests given Order.reverse(Order[Int])).order)
  checkAll("Order.reverse(Order.reverse(Order[Int]))", (OrderTests given Order.reverse(Order.reverse(Order[Int]))).order)
  checkAll("Order.fromLessThan[Int](_ < _)", (OrderTests given Order.fromLessThan[Int](_ < _)).order)
  ...
  checkAll("subsetPartialOrder[Int]", (PartialOrderTests given subsetPartialOrder[Int]).partialOrder)

  {
    ...
    checkAll("fromPartialOrdering[Int]", (PartialOrderTests given PartialOrder.fromPartialOrdering[Set[Int]]).partialOrder)
  }
  ...
  test("sign . toDouble . partialComparison = sign . partialCompare") {
    check2 { (x: Set[Int], y: Set[Int]) =>
      val found = subsetPartialOrder[Int].partialComparison(x, y).map(x => math.signum(x.toDouble))
      val expected = Some(subsetPartialOrder[Int].partialCompare(x, y)).filter(d => !d.isNaN).map(math.signum)
