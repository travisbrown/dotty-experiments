...
trait UnorderedFoldableLaws[F[_]] given (F: UnorderedFoldable[F]) {
  def unorderedFoldConsistentWithUnorderedFoldMap[A](fa: F[A]) given CommutativeMonoid[A]: IsEq[A] =
...
object UnorderedFoldableLaws {
  def apply[F[_]] given UnorderedFoldable[F]: UnorderedFoldableLaws[F] =
    new UnorderedFoldableLaws[F] {}
}
