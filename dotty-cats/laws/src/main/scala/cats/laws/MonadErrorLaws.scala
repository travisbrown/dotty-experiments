...
trait MonadErrorLaws[F[_], E] given (F: MonadError[F, E]) extends ApplicativeErrorLaws[F, E] with MonadLaws[F] {
...
object MonadErrorLaws {
  def apply[F[_], E] given MonadError[F, E]: MonadErrorLaws[F, E] =
    new MonadErrorLaws[F, E] with ApplicativeErrorLaws[F, E] with SemigroupalLaws[F] with MonadLaws[F] with FlatMapLaws[F] with ApplicativeLaws[F] with ApplyLaws[F] with FunctorLaws[F] with InvariantLaws[F]
}
