...
trait ApplicativeErrorLaws[F[_], E] given (F: ApplicativeError[F, E]) extends ApplicativeLaws[F] {
...
object ApplicativeErrorLaws {
  def apply[F[_], E] given ApplicativeError[F, E]: ApplicativeErrorLaws[F, E] =
    new ApplicativeErrorLaws[F, E] with ApplicativeLaws[F] with ApplyLaws[F] with FunctorLaws[F] with InvariantLaws[F] with SemigroupalLaws[F]
}
