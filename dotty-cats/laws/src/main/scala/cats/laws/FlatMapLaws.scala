...
import io.circe.cats.{ContravariantSemigroupal, FlatMap}
 ...
 */
trait FlatMapLaws[F[_]] given (F: FlatMap[F]) extends ApplyLaws[F] {
  ...
  private[laws] def eqTuple[A, B] given (A: Eq[F[A]], B: Eq[F[B]]): Eq[F[(A, B)]] =
    the[ContravariantSemigroupal[Eq]].composeFunctor[F].product(A, B)
}

object FlatMapLaws {
  def apply[F[_]] given FlatMap[F]: FlatMapLaws[F] =
    new FlatMapLaws[F] with ApplyLaws[F] with SemigroupalLaws[F] with FunctorLaws[F] with InvariantLaws[F]
}
