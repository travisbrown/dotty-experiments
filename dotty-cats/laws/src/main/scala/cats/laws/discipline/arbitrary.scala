...
import io.circe.cats.{Eval, Id, Representable, Show}
import io.circe.cats.data._
import io.circe.cats.data.NonEmptyList.ZipNonEmptyList
import io.circe.cats.data.NonEmptyVector.ZipNonEmptyVector
...
object arbitrary extends ArbitraryInstances0 {
  ...
  */
  ...
  implicit def catsLawsArbitraryForEval[A: Arbitrary]: Arbitrary[Eval[A]] =
    Arbitrary(
      Gen.oneOf(Arbitrary.arbitrary[A].map(a => Eval.now(a)),
                Arbitrary.arbitrary[() => A].map(f => Eval.later(f())),
                Arbitrary.arbitrary[() => A].map(f => Eval.always(f())))
    )
  ...
  //  Arbitrary(Arbitrary.arbitrary[Eq[A]].map(Eq.catsKernelEquivForEq(_)))

  implicit def catsLawsArbitraryForPartialOrder[A: Arbitrary]: Arbitrary[PartialOrder[A]] =
    Arbitrary(
      Arbitrary.arbitrary[Int => Double].map(
        f =>
          new PartialOrder[A] {
  ...
  //  Arbitrary(Arbitrary.arbitrary[PartialOrder[A]].map(PartialOrder.catsKernelPartialOrderingForPartialOrder(_)))
  ...
  implicit def catsLawsArbitraryForSortedMap[K: Arbitrary: Order, V: Arbitrary]: Arbitrary[SortedMap[K, V]] =
    Arbitrary(Arbitrary.arbitrary[Map[K, V]].map(s => SortedMap.empty[K, V](implicitly[Order[K]].toOrdering) ++ s))
  ...
  implicit def catsLawsArbitraryForSortedSet[A: Arbitrary: Order]: Arbitrary[SortedSet[A]] =
    Arbitrary(Arbitrary.arbitrary[Set[A]].map(s => SortedSet.empty[A](implicitly[Order[A]].toOrdering) ++ s))
  ...
  //  Arbitrary(Arbitrary.arbitrary[Order[A]].map(Order.catsKernelOrderingForOrder(_)))
