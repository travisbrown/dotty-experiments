...
object eq {
   ...
   */
  implicit def catsLawsEqForFn2[A, B, C](implicit A: Arbitrary[A], AE: ExhaustiveCheck[A], B: Arbitrary[B], BE: ExhaustiveCheck[B], C: Eq[C]): Eq[(A, B) => C] =
    Eq.by((_: (A, B) => C).tupled) given catsLawsEqForFn1Exhaustive
