 ...
 */
trait ApplicativeLaws[F[_]] given (F: Applicative[F]) extends ApplyLaws[F] {
...
object ApplicativeLaws {
  def apply[F[_]] given Applicative[F]: ApplicativeLaws[F] =
    new ApplicativeLaws[F] with ApplyLaws[F] with FunctorLaws[F] with InvariantLaws[F] with SemigroupalLaws[F]
}
