...
object CatsBoilerplate {
  ...
  val header = "// auto-generated boilerplate by /project/CatsBoilerplate.scala" // TODO: put something meaningful here?
  ...
  object GenApplyArityFunctions extends Template {
    def filename(root: File) = root / "io" / "circe" / "cats" / "ApplyArityFunctions.scala"
    ...
    def content(tv: TemplateVals) = {
      ...
      | */
      |private[cats] trait ApplyArityFunctions[F[_]] { self: Apply[F] =>
      |  def tuple2[A, B](f1: F[A], f2: F[B]): F[(A, B)] = Semigroupal.tuple2(f1, f2) given (self, self)
        -  /** @group ApArity */
        -  def ap$arity[${`A..N`}, Z](f: F[(${`A..N`}) => Z])($fparams):F[Z] = $apply
        -  /** @group MapArity */
        -  def map$arity[${`A..N`}, Z]($fparams)(f: (${`A..N`}) => Z): F[Z] = Semigroupal.map$arity($fparams)(f) given (self, self)
        -  /** @group TupleArity */
        -  def tuple$arity[${`A..N`}, Z]($fparams): F[(${`A..N`})] = Semigroupal.tuple$arity($fparams) given (self, self)
      |}
      """
    }
  }
  ...
  object GenSemigroupalArityFunctions extends Template {
    def filename(root: File) = root / "io" / "circe" / "cats" / "SemigroupalArityFunctions.scala"
    override def range = 2 to maxArity
    def content(tv: TemplateVals) = {
      import tv._
      ...
      val nestedProducts =
        (0 until (arity - 2)).foldRight(s"F.product(f${arity - 2}, f${arity - 1})")((i, acc) => s"F.product(f$i, $acc)")
      val `nested (a..n)` =
      ...
      | */
      |private[cats] trait SemigroupalArityFunctions {
        -  /** @group MapArity */
        -  def map$arity[F[_], ${`A..N`}, Z]($fparams)(f: (${`A..N`}) => Z) given (F: Semigroupal[F], FF: Functor[F]): F[Z] =
        -    FF.map($nestedProducts) { case ${`nested (a..n)`} => f(${`a..n`}) }
        -  /** @group ContramapArity */
        -  def contramap$arity[F[_], ${`A..N`}, Z]($fparams)(f: Z => (${`A..N`})) given (F: Semigroupal[F], FC: Contravariant[F]): F[Z] =
        -    FC.contramap($nestedProducts) { z => val ${`(a..n)`} = f(z); ${`nested (a..n)`} }
        -  /** @group ImapArity */
        -  def imap$arity[F[_], ${`A..N`}, Z]($fparams)(f: (${`A..N`}) => Z)(g: Z => (${`A..N`})) given (F: Semigroupal[F], FI: Invariant[F]): F[Z] =
        -    FI.imap($nestedProducts) { case ${`nested (a..n)`} => f(${`a..n`}) } { z => val ${`(a..n)`} = g(z); ${`nested (a..n)`} }
        -  /** @group TupleArity */
        -  def tuple$arity[F[_], ${`A..N`}]($fparams) given Semigroupal[F], Invariant[F]: F[(${`A..N`})] =
        -    imap$arity($fargsS)((${`_.._`}))(identity)
        -  /** @group TraverseArity */
        -  def traverse$arity[F[_], G[_], ${`A..N`}, Z]($fparams)(f: (${`A..N`}) => G[Z]) given (F: Semigroupal[F], FT: Traverse[F], G: Applicative[G]): G[F[Z]] =
        -    FT.traverse($nestedProducts) { case ${`nested (a..n)`} => f(${`a..n`}) }
      |}
      """
    }
  }
  ...
  object GenTupleSemigroupalSyntax extends Template {
    def filename(root: File) = root / "io" / "circe" / "cats" / "syntax" / "TupleSemigroupalSyntax.scala"
    ...
    def content(tv: TemplateVals) = {
      ...
      val contramap =
        if (arity == 1)
          s"def ($tupleTpe) contramap[Z](f: Z => (${`A..N`})) given (F: Contravariant[F]): F[Z] = F.contramap($tupleArgs)(f)"
        else
          s"def ($tupleTpe) contramapN[Z](f: Z => (${`A..N`})) given Contravariant[F], Semigroupal[F]: F[Z] = Semigroupal.contramap$arity($tupleArgs)(f)"
      ...
      |
      |import io.circe.cats.{Contravariant, Semigroupal}
      |
      |private[cats] trait TupleContravariantSemigroupalSyntax {
        - given Tuple${arity}Ops[F[_], ${`A..N`}] {
        -   $contramap
        - }
      |}
      """
